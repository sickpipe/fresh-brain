...
1. **Evenrail projects:** Fetch current record, update `notes` field to reflect current state — clear resolved issues, add recent work summary, update status if changed
...
Create a session note in the brain:
...
The "Suggested next step" block is a launching prompt, not a one-line afterthought. Treat it as the briefing that lets the next session start cold and move immediately. Pi-inspired structured handoff format (adopted 2026-05-24).
...
5. **Thin entries are fine** — if the session was trivial (no real work), skip Steps 1 and 3 but still write a one-liner session note. Gaps in the log are worse than thin entries.
6. **Be concrete** — file paths, decisions, current state. Not narrative.
7. **Role references, not character names** — in session notes and ship logs, reference team members by role or slug (e.g., "the researcher," "backend-dev"), not by their themed display name (e.g., "Scotty," "Dwight"). This keeps records theme-agnostic and survives any future re-theme without creating inconsistencies.

## Loading previous session at bootstrap

At session start, load the most recent session note:

```
memory_list_recent(source_table=''session_notes'', limit=1, summary_only=false)
```

This gives the full handoff context (where we left off, suggested next step, projects touched) in one structured query. The structured "Suggested next step" block is the launching prompt — read its Context, Task, and Constraints fields and use them to brief the Captain on resume options before any work begins.',
